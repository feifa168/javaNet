...
public class NIOServer {
    ...
    public List<SocketChannel> getClients() { return clients; }
    /**
     * initServer方法中:
     * 1.创建Selector
    ...
    public static void main(String[] args) throws Throwable {
        ...
        Thread sendThread = new Thread(()->{
            while (true) {
                try {
                    int len = System.in.read(bt);
                    List<SocketChannel> clients = server.getClients();
                    for (SocketChannel client : clients) {
                        client.write(ByteBuffer.wrap(bt, 0, len));
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
